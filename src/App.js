...
function Image({ img, index, name }) {
  ...
  return (
    <group ref={ref}>
      <Text anchorX="center" position={[0, w / 1.5 / 2 + 1, 0]} scale={[10, 10, 10]} color="white">
        {name}
      </Text>
...
function Content() {
  ...
  const names = ["La noche estrellada", "El grito", "La Gioconda", "El nacimiento de Venus", "Pablo Picasso", "La Libertad guiando al pueblo"];
  return images.map((img, index) => (
    <Block key={index} factor={1} offset={index}>
      <Image key={index} index={index} img={img} name={names[index]} />
    </Block>
  ));
}
